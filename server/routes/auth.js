...
import AuthSchema from '../models/users/auth.js'
    ...
    if (!userExist) {
      ...
      if (userExist.otpCode === req.body.otpCode) {
        ...
        if (currentTime > otpExpirationTime) {
          ...
          res
            .status(200)
            .json({ status: 200, data: userExist, cookie: cookie });
        }
      } else {
    ...
    try {
      ...
      if (userExists) {
        ...
        if (comparePass) {
          ...
          res.status(200).json({ status: 200, message: "Logged In Successfully", cookie });
        } else {
...
router.post('/get-role', async (req, res) => {
  ...
  try {
    ...
    const findUserRole = await AuthSchema.findOne({ email: req.body.email });

    if (!findUserRole) {
      return res.json({ status: 200, roles: "" });
    } else {
      res.status(200).json({ status: 200, roles: findUserRole.roles });
    }
